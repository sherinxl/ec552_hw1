...
class CelloQuery:
    ...
    def get_results(self) -> int:
        ...
        if sys.platform == "linux" or sys.platform == "darwin":
            docker_cmd = [
                f"docker run --rm "
                f"-v {self.input_directory}:./input "
                f"-v {self.output_directory}:./output"
                f" -t cidarlab/cello-dnacompiler:latest java -classpath ./app.jar "
                f"org.cellocad.v2.DNACompiler.runtime.Main "
                f"-inputNetlist ./input/{self.verilog_file} "
                f"-options ./input/{self.compiler_options} "
                f"-userConstraintsFile ./input/{self.input_ucf} "
                f"-inputSensorFile ./input/{self.input_sensors} "
                f"-outputDeviceFile ./input/{self.output_device} "
                f"-pythonEnv python3 "
                f"-outputDir ./output"
            ]
        if sys.platform == "win32":
